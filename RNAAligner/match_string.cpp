...
int scoreNoIndelsAndMakeMatchString(Config &config, string &read, string &ref, int refStart, string &match){
	int score = 0;
	int mode = -1;
	int timeInMode = 0;
...
int makeGref(Config &config, string &ref, vector<int> &gaps, int refStartLoc, int refEndLoc, string &gref) {
	...
	for(unsigned int i=0; i<gaps.size(); i+=2){
		...
		if(i+2 < gaps.size()){
			...
			int rem = gap%config.GAPLEN;
			int lim = y+ config.GAPBUFFER +rem;

			int div = (gap-config.GAPBUFFER2)/config.GAPLEN;
			...
			for(int r=z-config.GAPBUFFER; r<z; r++, gpos++){
				gref.push_back(ref[r]);
			}
		}
	}
...
void fillUnlimited(Config &config, string &read, string &ref, int refStartLoc, int refEndLoc, long max[], vector<vector<vector<long> > > &packed) {
	int rows = read.size();
...
void traceback(Config &config, string &read, string &ref, int refStartLoc, int refEndLoc, int row, int col, int state, Result &r, vector<vector<vector<long> > > &packed) {
...
void fillLimited(Config &config, string &read, string &ref, int refStartLoc, int refEndLoc, int score, vector<int> &gaps, long max[], Result &r) {
	string gref;
	int grefLimit = (refEndLoc - refStartLoc) + 2 * config.SLOW_ALIGN_PADDING;
	//bool gapped = true;
	if((int)gaps.size() == 0) {
		//gapped = false;
		for(int i = refStartLoc-config.SLOW_ALIGN_PADDING; i < refEndLoc + config.SLOW_ALIGN_PADDING; i++) {
...
void makeMatchStringForSite(Config &config, SiteScore ss, string &read, int *sizes, int *sites, Result &r, string &whole_genome, int threadId) {
