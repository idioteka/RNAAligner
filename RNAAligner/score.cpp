...
int scoreZ2(Config &config, vector<int> &locs, int centerIndex, vector<int> &offsets, int num_approx_hits, int num_hits){
	...
	int maxLoc = center + config.MAX_INDEL2;
	int minLoc = max(0, center - config.MAX_INDEL);
	int score=0;
	int a0 = -1;
	int b0 = -1;
		...
		if(loc > 0) {//match
			if(loc == lastValue){//contiguous match
				...
				if(dif > config.MINGAP){
					int rem = dif % config.GAPLEN;
					int div = (dif - config.GAPBUFFER2) / config.GAPLEN;
				...
				score += (config.POINTS_MATCH + POINTS_INS_ARRAY_C[min(loc-lastLoc, 5)]);
...
int extendScore(Config &config, string &read, vector<int> &offsets, vector<int> &values, int center_index, vector<int> &loc_array, int num_hits, int num_approx_hits, string &whole_genome, bool &isDirtyStart, bool &isDirtyStop, int &startError, int &stopError) {
	...
	int max_loc = center_loc + config.MAX_INDEL2;
	...
	for(int i = 0, keynum = 0; i < num_hits; i++){
		...
		if(value >= min_loc && value <= max_loc){
			...
			for(int cloc = callbase+config.KEYLEN-1, rloc = refbase+cloc; cloc >= 0 && rloc >= 0 && rloc < (int) whole_genome.size(); cloc--, rloc--){
	...
	for(int i = 0; i < num_hits; i++){
		...
		if(value >= min_loc && value <= max_loc){
			...
			int callbase = offsets[i];
			int misses=0;
			for(int cloc = callbase+config.KEYLEN, rloc = refbase+cloc; cloc < (int) read.size() && rloc < (int) whole_genome.size(); cloc++, rloc++){
				int old = loc_array[cloc];
				if(old == refbase) {
					break;
				} //Already filled with present value
