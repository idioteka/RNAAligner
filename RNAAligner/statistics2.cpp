...
void calculateStatistics32(Result &r1, Result &r2, Statistic &statistics) {
...
void calculateStatistics22(Result &r1, Result &r2, Statistic &statistics) {
	...
	for(unsigned int i = 0; i < r1.gapArray.size(); i+=2) {
		...
		for(unsigned int j = 0; j < r2.gapArray.size(); j+=2) {
			if(overlap2(r2.gapArray[j], r2.gapArray[j+1], r1.gapArray[i], r1.gapArray[i+1])) {
				int tmp = calculateOverlap2(r2.gapArray[j], r2.gapArray[j+1], r1.gapArray[i], r1.gapArray[i+1]);
				sum += tmp;
				num_over++;
				overlaped = true;
