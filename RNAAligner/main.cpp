...
//  RNAAligner
//
//  Created by Josip Maric on 14/06/15.
...
void align(Config &config, int bestScores[], vector<int> &keys, string &read, vector<int> &offsets, int *sizes,
    ...
    while(!heap.isEmpty()) {
        ...
        int approx_hits = 0;
        int minsite = site - config.MAX_INDEL;
        int maxsite = site + config.MAX_INDEL2;
...
void processRead(Config &config, int *sizes, int *sites, string &r1, vector<Result> &resultsFinal, vector<Read> &aligned_reads, vector<Read> &unaligned_reads, string &whole_genome, int threadId, int br) {
    ...
    for(int i = 0; i < config.SPLIT_COUNT; i++) {
        ...
        if(precounts[0] >= hitsCutoff && prescores[0] >= qscoreCutoff) {
            align(config, bestScores, read_keys_final, read, offsets, sizes, sites, results, all_bases_covered, max_score, quick_max_score, fully_defined, 0, whole_genome);
        }
        if(precounts[1] >= hitsCutoff && prescores[1] >= qscoreCutoff) {
            align(config, bestScores, keys_reversed, read_reverse, offsets_reversed, sizes, sites, results, all_bases_covered, max_score, quick_max_score, fully_defined, 1, whole_genome);
        }
...
void executeMethod(Statistic global_stat, Info &info, Config &config, int **res,  map<int, FastaRead> &read_names, map<int, Result> &correct_results,
    ...
    string build = to_string(config.BUILD_NUMBER);
    ...
    for(int i = 0; i < config.THREAD_NUM; i++) {
        ...
        datas3[i] = ThreadData3(i, sizes, sites, &reads, start, stop, &config, &whole_genome, &set_of_results[i], &aligned_reads[i], &unaligned_reads[i]);
    }
...
int main(int argc, char *argv[]) {
    ...
    if(read_index) {
        ...
        res = readIndex(whole_genome, genome_ref, config.INDEX_LOCATION, false, config);
    }
    else {
        cout << "Creating index..." << endl;
        res = createIndex(true, whole_genome, false, genome_ref, config.INDEX_LOCATION, config);
    }
    ...
    executeMethod(global_stat, info, config, res, read_names, correct_results, whole_genome, reads, read_index, tmp_unaligned_reads, tmp_aligned_reads, results);
